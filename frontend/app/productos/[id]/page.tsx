...
import { ShoppingCart, Smartphone, ArrowLeft, Minus, Plus, ChevronRight, Headphones } from "lucide-react";
...
export default function ProductoDetallePage() {
    ...
    useEffect(() => {
        // Cargar producto y luego los relacionados
            ...
            .then(res => {
                ...
                productosApi.getAll().then(allRes => {
                    ...
                    if (prod.tipoProducto === 'CELULAR') {
                        // Buscar accesorios compatibles o celulares de la misma marca
                        ...
                        // Si es accesorio, buscar celulares de su marca u otros accesorios
                        ...
                        <div>
                            <div className="bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden mb-4 relative aspect-square flex items-center justify-center">
                                ...
                                {producto.imagenes && producto.imagenes.length > 0 ? (
                                    ...
                                    producto.tipoProducto === 'CELULAR' ? <Smartphone className="text-gray-300" size={120} /> : <Headphones className="text-gray-300" size={120} />
                                )}
                            </div>
                                ...
                                <Link href={`/productos/${rel.id}`} key={rel.id} className="group flex flex-col bg-white border border-gray-100 rounded-2xl p-4 hover:shadow-xl hover:border-blue-100 transition-all hover:-translate-y-1">
                                    <div className="relative aspect-square w-full rounded-xl bg-gray-50 mb-4 overflow-hidden flex items-center justify-center p-6">
                                        ...
                                        {rel.imagenes && rel.imagenes.length > 0 ? (
                                            ...
                                            rel.tipoProducto === 'CELULAR' ? <Smartphone className="w-12 h-12 text-gray-300 group-hover:scale-110 transition-transform duration-500" /> : <Headphones className="w-12 h-12 text-gray-300 group-hover:scale-110 transition-transform duration-500" />
                                        )}
                                    </div>
